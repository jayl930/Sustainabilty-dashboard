...
# Neo4j configuration
neo4j_url = "neo4j+s://e19e6ad5.databases.neo4j.io:7687"
...
with title_col:
    st.title("Sustainability knowledge graph")
...
if user_input:
    ...
    with col1:
        if st.session_state["system_msgs"]:
            for i in range(len(st.session_state["system_msgs"]) - 1, -1, -1):
                ...
                message(
                    st.session_state["user_msgs"][i], is_user=True, key=str(i) + "_user"
                )
    ...
    with col3:
        if database_results:
            st.text_area(
                "Last Database Results", database_results, key="_database", height=240
            )
